...
class MinesweeperAI():
            ...
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    if (i, j) in self.safes:
                        ...
                        known_mines_around_cell += 1
                    else:
                        neighbor_cells.add((i, j))
        if neighbor_cells:  ###
            new_sentence = Sentence(neighbor_cells, count - known_mines_around_cell)
                ...
                for sentence2 in self.knowledge:
                    if sentence1 != sentence2 and sentence1.cells.issubset(sentence2.cells) and sentence2.cells:
                        ...
                        if new_sentence_count >= 0: ###
                            ...
                            if new_concluded_sentence not in self.knowledge and new_concluded_sentence not in new_sentences:  ###
                                new_sentences.append(new_concluded_sentence)
                                changed = True
    ...
    def make_random_move(self):
        ...
        for i in range(self.height):
            for j in range(self.width):
                board.add((i, j))
        possible_moves_set = ((board) - (self.moves_made)) - (self.mines)
        if possible_moves_set:
            for possible_move in possible_moves_set:
